    ...
    <div class="mark-table">
      ...
      <div class="mark-table__table">
        <div v-for="client in filterClients" :key="client.id" class="longCard">
          ...
          <span class="longCard__title" @click="openClientData(client)">{{ client.client_child_fio }}</span>
          <ul class="longCard__list">
            <li class="longCard__list-item" @click.prevent.right="rightClick(client, cell, $event)"
              @dblclick="toggleMark(client, cell, currentGroupOrIndividual)" v-for="cell in client.days"
              :key="cell.id">
              <span class="listItem__dot-mask" :class="{ listItem__dot: cell.isMarked }"></span>
            </li>
          </ul>
          <div class="moneyTrue" @click="openPaymentDialog(client)">
...
export default {
  ...
  methods: {
    ...
    toggleMark(client, cell, currentGroupOrIndividual) {
